...
export class SupervisarAtletaDataComponent implements OnInit {
  ...
  async iniciar()
  {
    ...
    this.reportecontroller.get_AllDataUser(null, this.usuarioSeleccionado);
    ...
    console.log(this.reportecontroller.cambiosRealTime);
      ...
      for(let i=0; i<reporte.length; i++)
      {
        ...
        datoOxigeno = +arregloDatos[2].split(':')[1];
        objetoTabla= {fechaHora: reporte[i].horaexacta, oxigeno: datoOxigeno, ritmo: datoRitmo, temperatura: datoTemperatura};
